...
class DataSet(object):
  def __init__(self, dataset_path, problem_type, seqlen, smilen, need_shuffle = False):
  ...
  def parse_data(self, is_log, with_label=True): 
    fpath = self.dataset_path	
...
def run(dataset_path, problem_type, max_seq_len, max_smi_len, is_log):
  dataset = DataSet(
  ...
  XD, XT, Y = dataset.parse_data(is_log)
  XD = np.asarray(XD)
  XT = np.asarray(XT)
  Y = np.asarray(Y)
